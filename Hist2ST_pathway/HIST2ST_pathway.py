...
class Hist2ST(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, fig_size=112, label=None, 
                 dropout=0.2, n_pos=64, kernel_size=5, patch_size=7, n_pathways=1, 
        ...
        if self.zinb>0:
            if self.nb:
                self.hr=nn.Linear(dim, n_pathways)  
                self.hp=nn.Linear(dim, n_pathways)  
                #self.hr=nn.Linear(dim, n_genes)
                #self.hp=nn.Linear(dim, n_genes)
            else:
                self.mean = nn.Sequential(nn.Linear(dim, n_pathways), MeanAct())  
                self.disp = nn.Sequential(nn.Linear(dim, n_pathways), DispAct())  
                self.pi = nn.Sequential(nn.Linear(dim, n_pathways), nn.Sigmoid())  
    ...
    def training_step(self, batch, batch_idx):
        ...
        patch, center, pathway_scores, adj, *_ = batch  
        print(f"Training batch - pathway_scores shape: {pathway_scores.shape}")
        ...
        mse_loss = F.mse_loss(pred, pathway_scores)  
        self.log('pathway_mse_loss', mse_loss,on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = F.mse_loss(pred.squeeze(0), pathway_scores.squeeze(0))  
        self.log('valid_pathway_loss', loss,on_epoch=True, prog_bar=True, logger=True)
        ...
        pred_np = pred.squeeze(0).cpu().numpy().T  # shape: (n_pathways, n_spots)
        target_np = pathway_scores.squeeze(0).cpu().numpy().T  # shape: (n_pathways, n_spots)
